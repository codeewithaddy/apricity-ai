...
export default function useApi() {
  ...
  return useMemo(() => ({
    ...
    ask: async (query, documentId, allowGeneral = false, chatId = null, createIfMissing = false, signal = null, inDepthMode = false, images = []) => {
      const token = localStorage.getItem('token');
      const selectedModel = localStorage.getItem('selectedModel') || 'gemini-2.0-flash';
      const headers = { 
        "Content-Type": "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {})
      };
      const res = await fetch(`${base}/api/rag/ask`, {
        method: "POST",
        headers,
        body: JSON.stringify({ query, documentId, allowGeneral, chatId, createIfMissing, inDepthMode, images, model: selectedModel }),
