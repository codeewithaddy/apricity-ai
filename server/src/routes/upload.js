...
router.post('/', upload.single('pdf'), async (req, res) => {
  try {
    ...
    // Keep local file on disk so the server endpoint /api/upload/:id/file can always stream it reliably
...
router.get('/:id/file', async (req, res) => {
  ...
  const token = (authHeader && authHeader.split(' ')[1]) || queryToken;
    ...
    // Helper to resolve existing file path on disk (including legacy server/server/uploads)
    let validPath = null;
    if (doc.storagePath) {
      if (fs.existsSync(doc.storagePath)) {
        validPath = doc.storagePath;
      } else {
        // Try fixing legacy nested path (e.g. server/server/uploads -> uploads)
        const filename = path.basename(doc.storagePath);
        const candidate1 = path.join(uploadsDir, filename);
        const candidate2 = path.join(process.cwd(), 'server', 'uploads', filename);
        if (fs.existsSync(candidate1)) validPath = candidate1;
        else if (fs.existsSync(candidate2)) validPath = candidate2;
      }
    }

    // If local file exists, stream it directly with proper headers for inline display
    if (validPath) {
      res.setHeader('Content-Type', doc.mimeType || 'application/pdf');
      res.setHeader('Content-Disposition', `inline; filename="${encodeURIComponent(doc.title || doc.filename || 'document.pdf')}"`);
      return fs.createReadStream(validPath).pipe(res);
    }

    // Fallback: If Cloudinary URL exists and local file is missing, redirect to Cloudinary
