...
router.post('/ask', optionalAuth, async (req, res) => {
  try {
    const { query, documentId, allowGeneral = true, chatId, createIfMissing, images = [], inDepthMode = false, model: requestModel } = req.body;
    if (!query) return res.status(400).json({ error: 'query required' });
    const targetModel = requestModel || process.env.GEMINI_MODEL || 'gemini-2.0-flash';
    
    console.log('Ask request:', { query: query.slice(0, 50), documentId, chatId, model: targetModel, hasImages: images.length > 0, userId: req.user?._id || 'guest' });
  ...
  : `You are Gini, an AI tutor from Apricity.ai created by Adarsh Gupta. Your role is to help students learn effectively.\n\nKey guidelines:\n- Provide **medium-length answers** (2–4 paragraphs; do NOT give just a short summary or 1–2 sentences)\n- Be clear and direct - get to the point quickly\n- Use simple language with relevant examples when needed\n- Break down complex concepts into understandable parts\n- Be conversational and natural - avoid robotic or repetitive introductions\n- Only mention your identity if directly asked "who are you" or "what are you" and when you get greeting\n- Focus on answering the question directly rather than introducing yourself\n- Remember the conversation context and refer back to previous messages when relevant\n\nAlways be friendly, encouraging, and patient.`;
    ...
    const answer = images.length > 0
      ? await generateTextWithImages({ prompt, system, imageUrls: images, model: targetModel, temperature: 0.7 })
      : await generateText({ prompt, system, model: targetModel, temperature: 0.7 });
  ...
  let docs = [];
  if (documentId) {
    // Guard: local: IDs are guest-only temp IDs never saved to DB — fall back to general AI mode
    if (typeof documentId === 'string' && documentId.startsWith('local:')) {
      console.log('Guest local PDF ID detected — falling back to general mode');
      return res.json({ 
        answer: "Your PDF was uploaded as a guest and hasn't been saved to our servers. To use PDF-based Q&A, please **sign up for a free account** — your PDFs will be stored and fully searchable.", 
        citations: [], 
        usedGeneral: true,
        isGuest: true
      });
  ...
  answer = await generateText({ prompt, system, model: targetModel, temperature: 0.7 });
